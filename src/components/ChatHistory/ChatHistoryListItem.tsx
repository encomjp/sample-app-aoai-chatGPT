...
export const ChatHistoryListItemCell: React.FC<ChatHistoryListItemCellProps> = ({ item, onSelect }) => {
  ...
  const dialogContentProps = {
    ...
    title: 'Sind Sie sicher, dass Sie diesen Artikel löschen möchten?',
  ...
  const handleSaveEdit = async (e: any) => {
    ...
    if (editTitle == item.title) {
      setErrorRename('Fehler: Geben Sie einen neuen Titel ein, um fortzufahren')
      setTimeout(() => {
      ...
      {edit ? (
        <>
          <Stack.Item style={{ width: '100%' }}>
            <form aria-label="Titelformular bearbeiten" onSubmit={e => handleSaveEdit(e)} style={{ padding: '5px 0px' }}>
    ...
    <div className={styles.listContainer} data-is-scrollable>
        ...
        group =>
          group.entries.length > 0 && (
            <Stack
              ...
              aria-label={`Chatverlaufsgruppe: ${group.month}`}>
              <Stack aria-label={group.month} className={styles.chatMonth}>
                {formatMonth(group.month)}
              </Stack>
              <List
                aria-label={`Chat-Verlaufsliste`}
                items={group.entries}
      ...
      {showSpinner && (
        <div className={styles.spinnerContainer}>
          <Spinner size={SpinnerSize.small} aria-label="Laden von mehr Chatverlauf" className={styles.spinner} />
        </div>
      )}
    </div>
  )
