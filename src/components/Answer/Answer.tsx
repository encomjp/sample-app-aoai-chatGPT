...
export const Answer = ({ answer, onCitationClicked, onExectResultClicked }: Props) => {
    ...
    return (
      <>
        ...
        <Stack tokens={{ childrenGap: 4 }}>
          ...
          <Checkbox
            label="Quellenangaben sind falsch."
          ...
          <Checkbox
            ...
            onChange={updateFeedbackList}></Checkbox>
          <Checkbox
            label="Unzutreffend oder irrelevant."
  ...
  const ReportInappropriateFeedbackContent = () => {
    return (
      <>
        <div>
          Inhalt ist <span style={{ color: 'red' }}>*</span>
        </div>
        <Stack tokens={{ childrenGap: 4 }}>
          <Checkbox
            label="Hassreden, Stereotypisierung, Herabwürdigung"
            id={Feedback.HateSpeech}
            defaultChecked={negativeFeedbackList.includes(Feedback.HateSpeech)}
            onChange={updateFeedbackList}></Checkbox>
          <Checkbox
            label="Gewalttätig: Gewaltverherrlichung, Selbstbeschädigung"
            id={Feedback.Violent}
            defaultChecked={negativeFeedbackList.includes(Feedback.Violent)}
            onChange={updateFeedbackList}></Checkbox>
          <Checkbox
            label="Sexuell: expliziter Inhalt, Grooming"
            id={Feedback.Sexual}
            defaultChecked={negativeFeedbackList.includes(Feedback.Sexual)}
            onChange={updateFeedbackList}></Checkbox>
          <Checkbox
            label="Manipulativ: hinterhältig, emotional, aufdringlich, schikanös"
  ...
  return (
    <>
      <Stack className={styles.answerContainer} tabIndex={0}>
            ...
            <Stack.Item className={styles.answerHeader}>
              {FEEDBACK_ENABLED && answer.message_id !== undefined && (
                <Stack horizontal horizontalAlign="space-between">
                  <ThumbLike20Filled
                    aria-hidden="false"
                    aria-label="Diese Antwort gefällt mir"
                  ...
                  <ThumbDislike20Filled
                    aria-hidden="false"
                    aria-label="Diese Antwort gefällt mir nicht"
        ...
        <Stack horizontal className={styles.answerFooter}>
              ...
              <Stack style={{ width: '100%' }}>
                <Stack horizontal horizontalAlign="start" verticalAlign="center">
                  <Text
                    ...
                    aria-label="Offene referenze"
                    tabIndex={0}
                    role="button">
                    <span>
                      {parsedAnswer.citations.length > 1
                        ? parsedAnswer.citations.length + ' referenzen'
                        : '1 referenz'}
                    </span>
                  </Text>
                  <FontIcon
          ...
          <Stack.Item className={styles.answerDisclaimerContainer}>
            <span className={styles.answerDisclaimer}>KI-generierte Inhalte können fehlerhaft sein.</span>
          </Stack.Item>
      ...
      <Dialog
        ...
        <Stack tokens={{ childrenGap: 4 }}>
          <div>Ihr Feedback wird diese Erfahrung verbessern.</div>

          {!showReportInappropriateFeedback ? <UnhelpfulFeedbackContent /> : <ReportInappropriateFeedbackContent />}

          <div>Durch das Drücken von "Senden" wird Ihr Feedback für den Anwendungsinhaber sichtbar.</div>
